...
#include "Interpolation.h"	// Include interpolation for movement
...
public:
	...
	inline void FreeMove(double &delta)
	{
		if (ControlActive())	// If the a control is active...
		{
			Interpolation::Linestep(_trans._pos, _velocity, GetCurrentLookVectorV(), _speed, delta);	// Interpolate camera
		}
	}
