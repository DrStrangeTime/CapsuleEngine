...
public:
	...
	inline void Create(std::vector<GLuint> shader_programs, size_t width, size_t height, GLuint sample_res)
	{
		...
		for (unsigned int i = 0; i < sample_res; ++i)	// Iterate through each sample...
		{
			...
			scale = Interpolation::lerpf(0.1f, 1.0f, scale * scale);		// Interpolate the scale
	...
	inline virtual void Render()
	{
		...
		glUniformMatrix4fv(_u_projection, 1, GL_FALSE, glm::value_ptr(Content::_map->GetPlayerController()->GetProjectionMatrix()));	// Assign camera projection uniform data
