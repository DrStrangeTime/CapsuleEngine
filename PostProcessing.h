...
class FXAA : public PostFX
{
private:
	unsigned int _scene;

	unsigned int _u_filterTexture;
	unsigned int _u_inverseFilterTextureSize;
	unsigned int _u_fxaaSpanMax;
	unsigned int _u_fxaaReduceMin;
	unsigned int _u_fxaaReduceMul;
public:
	inline FXAA(std::vector<GLuint> shader_programs)
	{
		Create(shader_programs);
	}

	inline ~FXAA()
	{
		glDeleteTextures(1, &_scene);
	...
	inline void Create(std::vector<GLuint> shader_programs)
	{
		...
		_fbo = new Fbo(1920, 1080, { new FboAttachment(1920, 1080, GL_RGBA, GL_RGBA, GL_FLOAT, GL_COLOR_ATTACHMENT0) }, false);
	}
...
private:
	...
	unsigned int	_u_texturemap_h;
	unsigned int	_u_texturemap_v;
	unsigned int	_u_blurres_h;
	unsigned int	_u_blurres_v;
	unsigned int	_u_texture_map;
	unsigned int	_u_blur_resolution;
...
public:
	unsigned int _u_contrast;
	unsigned int _u_brightness;
...
private:
	...
	unsigned int _u_lsm; // lightspacematrix uniform location
	unsigned int _u_mat; // model matrix uniform location
	unsigned int _u_texturemap_h; // the horizontal blur shadowmap texture 
	unsigned int _u_texturemap_v; // the vertical blur shadowmap texture 
	unsigned int _u_blurres_h; // the horizontal blur shadowmap texture 
	unsigned int _u_blurres_v; // the vertical blur shadowmap texture 
...
public:
	...
	inline void Create(std::vector<GLuint> shader_programs, size_t shadow_resolution)
	{
		...
		*/
		...
		_u_mat = glGetUniformLocation(shader_programs[0], "model"); // load in the model matrix from the firstpass shader
	...
	inline virtual void Render()
	{
		...
		{ // DIRECTIONAL SHADOWS
			...
			light_space_matrix = light_projection * light_view * glm::inverse(Content::_map->GetPlayerController()->GetViewMatrix()); // calculate the lightSpaceMatrix
			...
			{
				glUniformMatrix4fv(_u_mat, 1, GL_FALSE, glm::value_ptr(Content::_map->GetPlayerController()->GetViewMatrix() * Content::_map->GetActors()[i]->GetMatrix())); // set the viewspace model matrix uniform

				if (Content::_map->GetActors()[i]->GetObjectType() == MESH)
...
private:
	...
	float _max_blur;
public:
	inline DOF(std::vector<GLuint> shader_programs)
	{
		Create(shader_programs);
	}
	...
	inline void Create(std::vector<GLuint> shader_programs)
	{
		...
		_fbo = new Fbo(1920, 1080, { new FboAttachment(1920, 1080, GL_RG32F, GL_RGBA, GL_FLOAT, GL_COLOR_ATTACHMENT0, true, true) }, false);
		_rbo = new Rbo(1920, 1080, GL_DEPTH_COMPONENT, GL_DEPTH_ATTACHMENT, 1);
	...
	inline void Render()
	{
		...
		{ // DIRECTIONAL SHADOWS
			glm::mat4 light_space_matrix = Content::_map->GetPlayerController()->GetProjectionMatrix() * Content::_map->GetPlayerController()->GetViewMatrix(); // calculate the lightSpaceMatrix

			glUseProgram(_shader_programs[0]); // bind the first pass shader

			glUniformMatrix4fv(_u_lsm, 1, GL_FALSE, glm::value_ptr(light_space_matrix)); // set the lightSpaceMatrix uniform

			glViewport(0, 0, 1920, 1080); // set the viewport size to the resolution of the shadow map
			...
			for (unsigned int i = 0; i < Content::_map->GetActors().size(); i++)
			{
				glUniformMatrix4fv(_u_mat, 1, GL_FALSE, glm::value_ptr(Content::_map->GetActors()[i]->GetMatrix())); // set the viewspace model matrix uniform
...
{
private:
	unsigned int _u_view; // view matrix uniform location
	unsigned int _u_proj; // projection matrix uniform location
	unsigned int _u_mat; // model matrix uniform location
	unsigned int _u_objtype; // object type (light scatter caster or not) uniform location

	unsigned int _u_exposure; // light scatter exposure uniform location
	unsigned int _u_decay; // light scatter decay uniform location
	unsigned int _u_density; // light scatter density uniform location
	unsigned int _u_weight; // light scatter weight uniform location
	unsigned int _u_lightScreenPos; // light scatter screenspace light position uniform location
	unsigned int _u_lightScatterTexture; // light scatter texture uniform location
	unsigned int _u_noise_texture;
	unsigned int _u_horizontal_texturemap;
	unsigned int _u_horizontal_blurres;
	unsigned int _u_vertical_texturemap;
	unsigned int _u_vertical_blurres;
...
public:
	...
	inline void Create(std::vector<GLuint> shader_programs, size_t width, size_t height)
	{
...

		{
			...
			_u_mat = glGetUniformLocation(shader_programs[0], "model"); // load in the model matrix uniform from the light scatter shader
	...
	inline virtual void Render()
	{
		...
		glm::mat4 projection = Content::_map->GetPlayerController()->GetProjectionMatrix(); // camera projection
		glm::mat4 view = Content::_map->GetPlayerController()->GetViewMatrix(); // camera view
		...
		model = glm::translate(glm::vec3(0.0f, 4.0f, 0.3f)) * glm::rotate(0.0f, glm::vec3(0.0f, 1.0f, 0.0f)) * glm::scale(glm::vec3(2.0f, 2.0f, 2.0f));
		glUniformMatrix4fv(_u_mat, 1, GL_FALSE, glm::value_ptr(model)); // set the model matrix uniform
		...
		{
			glUniformMatrix4fv(_u_mat, 1, GL_FALSE, glm::value_ptr(view * a->GetMatrix())); // set the model matrix uniform	
